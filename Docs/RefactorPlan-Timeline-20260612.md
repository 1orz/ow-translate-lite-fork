...
- **做什么**：用录制模式采集 5 类真实场景帧序列并人工标注期望输出（该场景应翻译哪些消息、什么顺序）：
  1. 韩语普通新消息（单条、短句为主）；
...
- **完成标准**：5 个 case 入库（建议 `Tools/ReplayLab/fixtures/` 或类似位置），含期望输出标注；基线指标记录在案，作为后续所有阶段的对比基准。
...
- **做什么**：实现核心算法：输入「本帧有序可见行列表」与「Timeline 尾部窗口」，做全局序列对齐（Needleman-Wunsch 动态规划；匹配分 = T1.1 后的 `TextSimilarityScore` + speaker 匹配加成，gap penalty 初值在 replay 上调）。输出三类结果：
...
- **如何测试**：ReplayLab 上跑全部 golden case：case 2（多人陆续发言）零漏翻、顺序正确；case 4（字符抖动）零重复；case 3（开历史）多条消息全部进入新消息列表。
- **完成标准**：5 个 golden case 全部达到期望输出；坏帧注入测试（人工在序列中插入乱码帧）后 Timeline 不被污染、后续帧恢复正常。
...
- **做什么**：
  ...
  4. `ResetChatCycle` / `ChatHiddenReset` 语义重新映射到 Timeline（聊天隐藏超时 → 清 Timeline 或仅清"确认中"状态——按 D8 的电平触发设计，**该语义与 T4.1 联动，先保持等价行为**）。
...
| T0.3 golden cases | P0 | T0.1, T0.2 | 部分场景可延后补录 | 5 case 入库 + 基线指标 |
...
| T2.2 对齐检测器 | P0 | T1.1, T2.1；验证依赖 T0.3 | — | golden case 全绿 + 坏帧免疫 |
...
1. **回放回归（每次提交必跑）**：ReplayLab 跑全部 golden case + 韩语对照组，输出三指标：漏翻数 / 重复数 / 乱序数。验收线：case 1–5 全部达到标注期望；任何提交不得使指标劣于上一提交。
...
9. `feat(timeline): 序列对齐检测器`（T2.2，纯新增 + 单测）
10. `refactor(coordinator)!: 接入 Timeline 对齐，移除 anchor/TTL 判重机制群`（T2.3，**本计划最大提交，打 tag**）
...
- 新消息多帧共识后入队：连续两帧一致(或 jamo 距离≤1)立即发车，分歧再等一帧
  投票，最多 2 个确认帧。送翻文本=多帧共识文本。
...
【验收标准】
- 5 个 golden case（韩语短消息/多人陆续发言/开聊天历史/OCR 字符抖动/空格抖动）
  全部达到标注期望：零漏翻、零重复、零乱序。
